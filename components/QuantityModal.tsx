...
export const QuantityModal: React.FC<QuantityModalProps> = ({ product, isOpen, onConfirm, onCancel }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className={`bg-white rounded-xl shadow-2xl w-full max-w-md p-6 transform transition-all scale-100 border-t-8 ${product.isNew ? 'border-purple-500' : 'border-blue-500'}`}>
        
        <h2 className="text-xl font-bold text-gray-800 mb-2">
            {product.isNew ? 'Adăugare Produs Nou' : 'Confirmare Intrare'}
        </h2>
        
        <div className={`p-3 rounded-lg border mb-4 ${product.isNew ? 'bg-purple-50 border-purple-100' : 'bg-blue-50 border-blue-100'}`}>
          ...
          <p className="font-mono text-lg font-bold text-gray-900 break-words">{product.code}</p>
        ...
        <div className="grid grid-cols-3 gap-2 mb-4 bg-gray-50 p-2 rounded-lg">
            <div className="text-center">
                <span className="block text-[10px] text-gray-500 uppercase">Scriptic</span>
                <span className="block text-lg font-bold text-gray-400">{product.scripticStock}</span>
            </div>
            <div className="text-center border-l border-r border-gray-200">
                <span className="block text-[10px] text-gray-500 uppercase">Faptic Vechi</span>
             ...
             <div className="text-center bg-green-50 rounded">
                <span className="block text-[10px] text-green-700 uppercase font-bold">Total Nou</span>
        ...
        <div className="mb-6">
          ...
          <div className="flex gap-2">
             <input
                ...
                className={`flex-1 border-2 rounded-lg p-3 text-2xl font-bold text-center focus:outline-none focus:ring-4 ${product.isNew ? 'border-purple-500 focus:ring-purple-500/20' : 'border-blue-500 focus:ring-blue-500/20'}`}
             />
          </div>
        ...
        <div className="flex gap-3 justify-end">
          ...
          <button 
            ...
            className={`px-6 py-2.5 rounded-lg text-white font-bold shadow-lg hover:shadow-xl transition transform active:scale-95 ${product.isNew ? 'bg-purple-600 hover:bg-purple-700' : 'bg-blue-600 hover:bg-blue-700'}`}
          >
            Confirmă (Enter)
          </button>
        </div>
